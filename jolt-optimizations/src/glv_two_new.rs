...
// ============================================================================
// Vector scalar multiplication utilities (Dory protocol)
// ============================================================================

/// Precomputed data for v[i] += scalar * generators[i]
...
impl VectorScalarMulG1Data {
    pub fn new(generators: &[G1Projective], scalar: Fr) -> Self {
        let (coeffs, signs) = decompose_scalar_2d(scalar);
        let shamir_tables = generators
            .par_iter()
            .map(|gen| {
                let glv_bases = [*gen, glv_endomorphism(gen)];
                PrecomputedShamir2Table::new(&glv_bases)
            })
            .collect();
        
        Self { shamir_tables, coeffs, signs }
    }
}

/// Online: v[i] += scalar * generators[i]
...
/// Precomputed: v[i] += scalar * generators[i]
pub fn vector_scalar_mul_add_g1_precomputed(v: &mut [G1Projective], data: &VectorScalarMulG1Data) {
    assert_eq!(v.len(), data.shamir_tables.len());
    
    v.par_iter_mut()
        .zip(data.shamir_tables.par_iter())
        .for_each(|(vi, table)| {
            *vi += shamir_glv_mul_2d_precomputed(table, &data.coeffs, &data.signs);
        });
}

/// Convenience: v[i] += scalar * generators[i]
pub fn vector_scalar_mul_add_g1(
    v: &mut [G1Projective],
    generators: &[G1Projective],
    scalar: Fr,
) {
...
/// Precomputed data for v[i] = scalar * v[i] + generators[i]
...
/// Online: v[i] = scalar * v[i] + generators[i]
pub fn vector_scalar_mul_v_add_g_g1_online(
    ...
    v.par_iter_mut()
        ...
        .for_each(|(vi, gen)| {
            ...
            *vi = shamir_glv_mul_2d(&bases, &coeffs, &signs) + gen;
        });
}

/// Precomputed: v[i] = scalar * v[i] + generators[i]
pub fn vector_scalar_mul_v_add_g_g1_precomputed(
    ...
    v.par_iter_mut()
        ...
        .for_each(|(vi, gen)| {
            let bases = [*vi, glv_endomorphism(vi)];
            let table = PrecomputedShamir2Table::new(&bases);
            *vi = shamir_glv_mul_2d_precomputed(&table, &data.coeffs, &data.signs) + gen;
        });
}
